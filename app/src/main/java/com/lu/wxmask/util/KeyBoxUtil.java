...
public final class KeyBoxUtil {
    ...
    public static void showSoftInput(@NonNull final View view, final int flags) {
        InputMethodManager imm =
                (InputMethodManager) AppUtil.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
        if (imm == null) return;
    ...
    public static void hideSoftInputByToggle(final Activity activity) {
        ...
        if (Math.abs(delta) > 500 && KeyBoxUtil.isSoftInputVisible(activity)) {
            KeyBoxUtil.toggleSoftInput();
        }
        millis = nowMillis;
    }
